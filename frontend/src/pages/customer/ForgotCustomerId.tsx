...
export const ForgotCustomerId: React.FC = () => {
  const navigate = useNavigate();
  const { showToast } = useToast();
  const { theme, toggleTheme } = useTheme();
  const isDark = theme === 'dark';

  // Wizard steps: 1 = Email Input, 2 = OTP Verification, 3 = Customer ID Success
  ...
  const handleRequestOtp = async (e?: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${API_URL}${endpoints.auth.customerForgotIdRequestOtp}`, {
        email: email.trim(),
      });
  ...
  /* ────────────────── Step 2: Verify OTP & Retrieve Customer ID ────────────────── */
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${API_URL}${endpoints.auth.customerForgotIdVerifyOtp}`, {
        email: email.trim(),
        otp: otp.trim(),
      });

      if (res.data.success) {
        showToast('OTP verified successfully! Customer ID retrieved.', 'success');
  ...
  return (
    <div className="relative flex min-h-screen w-screen items-center justify-center bg-slate-50 dark:bg-slate-950 px-4 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md bg-white/80 dark:bg-slate-900/50 backdrop-blur-2xl rounded-3xl shadow-2xl shadow-slate-300/40 dark:shadow-black/60 border border-slate-200/80 dark:border-white/10 p-8 overflow-hidden transition-all">
          ...
          <>
            <div className="mb-6">
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400 mt-1.5 leading-relaxed">
                Enter your registered email address to verify your identity and recover your Customer ID.
              </p>
            </div>
        ...
        {/* ════════════════ STEP 3: SUCCESS CUSTOMER ID SCREEN ════════════════ */}
        {step === 3 && (
          <div className="text-center space-y-5">
            ...
            <div>
              <h2 className="text-xl font-extrabold text-slate-900 dark:text-slate-100">
                Customer ID Recovered Successfully
              </h2>
              <p className="text-xs text-slate-600 dark:text-slate-400 mt-1.5">
                Your registered Customer ID is shown below.
              </p>
            </div>
            ...
            <div className="text-left p-3.5 rounded-xl bg-slate-100/60 dark:bg-white/[0.03] border border-slate-200/60 dark:border-white/10 space-y-1 text-xs text-slate-600 dark:text-slate-400 font-medium">
              <div className="flex items-center gap-2">
                ...
                <span>You can now sign in using this Customer ID.</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
            ...
            <div className="flex flex-col gap-2.5 pt-2">
              ...
              >
                ...
                {copied ? 'Copied to Clipboard!' : 'Copy Customer ID'}
              </Button>
